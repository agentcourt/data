...
The import excluded `*.pid` files.  A scan found no `auth.json` files, no OpenClaw Codex homes, and no retained Pi or npm working trees in the copied run set.  The retained `run-batch.sh` contains the local command shape, including a reference to `$HOME/.codex/auth.json`, but it contains no token value.
