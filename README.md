...
Run the verifier from the repository root.  The command checks all published run sets.  It exits with a nonzero status if a checksum, manifest field, or closed-run invariant fails.

```sh
tools/verify.sh
```

The verifier checks `SHA256SUMS`, checks each run-level checksum file, confirms that every run has `status: ok` and `phase: closed`, and compares manifest fields with each run’s `run.json`.  It uses `sha256sum`, `jq`, `find`, and `diff`.
